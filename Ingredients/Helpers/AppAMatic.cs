...
namespace AppAMatic.Helper {

    static public class DataExtensions {

        static private string Connstring() {
            return ConfigurationManager.ConnectionStrings["AppAMatic"].ToString();
        }
        ...
        public static List<T> Query<T>(this IOrmModel me, params KeyValuePair<string, object>[] Constraints) where T : IOrmModel, new() {
            ...
            try {
                using (SqlConnection cn = new SqlConnection(Connstring())) {
                    ...
                    foreach (KeyValuePair<string, object> Constraint in Constraints) {
                        cmd.Parameters.AddWithValue(Constraint.Key, Constraint.Value);
                    }
                    SqlDataReader r = cmd.ExecuteReader();
                    while (r.Read()) {
                        Dictionary<string, object> row = r.ToDataRow();
